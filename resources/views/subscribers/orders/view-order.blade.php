        ...
        <div class="w-full mx-auto bg-white shadow-xl rounded-2xl p-8 border border-gray-100 space-y-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-6 text-gray-800">
                <div>
                    ...
                    <p class="text-base font-semibold">{{$order->project_title ?? 'N/A'}}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Request Type</p>
                    <p class="text-base font-semibold">{{$order->request_type ?? 'N/A'}}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Sub Services</p>
                    <p class="text-base font-semibold">{{$order->sub_service ?? 'N/A'}}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p>{{$subscribers->where('id', $order->created_by)->first()->first_name ?? 'N/A'}} {{$subscribers->where('id', $order->created_by)->first()->last_name ?? 'N/A'}}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Contact Number</p>
                    <p>{{$subscribers->where('id', $order->created_by)->first()->mobile_number ?? 'N/A'}}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Email</p>
                    <p>{{$subscribers->where('id', $order->created_by)->first()->email ?? 'N/A'}}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Pre Approve</p>
                    <p>{{$order->pre_approve ?? 'N/A'}}</p>
                </div>
                    ...
                    <div x-show="open" x-transition class="mt-4 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-800">
                            ...
                            <div>
                                ...
                                <p class="font-medium">{{ $brand->brand_name ?? 'N/A'}}</p>
                            </div>
                            <div>
                                <p class="text-gray-500 mb-1">Industry</p>
                                <p class="font-medium">{{ $brand->industry ?? $brand->other_industry ?? 'N/A'}}</p>
                            </div>
                            <div>
                        ...
                        <div class="mt-6 flex items-center gap-4">
                            ...
                            @if($order->rush)
                                <form method="POST" action="{{ route('credits-usage.approve', $order->id) }}">
                                    @csrf
                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 text-sm font-semibold rounded shadow-sm transition">
                                        ✅ Approve Ratings
                                    </button>
                                </form>
                            @endif
